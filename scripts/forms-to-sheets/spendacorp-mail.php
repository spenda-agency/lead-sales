...
##-----------------------------------------------------------------------------------------------------------------##
#
#  PHPメールプログラム　フリー版 ver2.0.3（Google Apps Script連携版） 最終更新日2026/05/31
#　改造や改変は自己責任で行ってください。
#
#  HP: http://www.php-factory.net/
#
#  ★★★ Google Apps Script連携機能（spenda forms-to-sheets 仕様） ★★★
#  問合せデータを「フォーム関連」Google スプレッドシートに自動保存します
#  設定値は同階層の spenda-config.php に分離 (リポにコミット禁止)
#
##-----------------------------------------------------------------------------------------------------------------##
...
$site_top = "https://spendacorp.com/";

//管理者のメールアドレス ※メールを受け取るメールアドレス(複数指定する場合は「,」で区切ってください 例 $to = "aa@aa.aa,bb@bb.bb";)
$to = "spendaob11@gmail.com,contact@spenda-c.com";
...
//----------------------------------------------------------------------
//  ★★★ Google Apps Script 連携設定 (spenda forms-to-sheets) ★★★
//----------------------------------------------------------------------
//
//  設定値ファイル (SPENDA_GAS_URL / SPENDA_FORM_SECRET) を読み込み
//  secret はリポにコミット禁止 → 同階層の spenda-config.php に分離:
//    <?php
//    define('SPENDA_GAS_URL',    'https://script.google.com/macros/s/<新/exec URL>/exec');
//    define('SPENDA_FORM_SECRET','<GAS Script Properties の FORM_SHARED_SECRET と同値>');
//
//  spenda-config.php が無い、もしくは定数が未定義なら連携は no-op (mail.php は通常稼働)
//
@include __DIR__ . '/spenda-config.php';

// フォーム ID (Sheet の form_id 列に入る値)
$SPENDA_FORM_ID = 'contact-html';
...
$BccMail = "contact@spenda-c.com";

// 管理者宛に送信されるメールのタイトル（件名）
$subject = "広告運用からのお問い合わせ";
...
$thanksPage = "https://spendacorp.com/thanks.html";
...
$require = array('お名前','会社名','お問合せ種別');

//----------------------------------------------------------------------
//  reCAPTCHA v3 設定
//----------------------------------------------------------------------

// reCAPTCHA v3 のシークレットキー
$recaptcha_secret_key = "6Lfkr4krAAAAAPcdLF8yuG6VVHHPVUYIVvVt8EaS";

// reCAPTCHA検証を有効にする(する=1, しない=0)
$use_recaptcha = 1;

// reCAPTCHAのスコア閾値（0.0〜1.0、高いほど厳しい）
...
携帯:090-3518-8075
E-mail:juito@spenda-c.com
URL: http://spendacorp.com/
──────────────────────
...
if(($confirmDsp == 0 || $sendmail == 1) && $empty_flag != 1){
	...
	if($use_recaptcha == 1){
		...
		if(!verifyRecaptcha($recaptcha_token, $recaptcha_secret_key, $recaptcha_score_threshold)){
			exit('reCAPTCHA検証に失敗しました。もう一度お試しください。');
		}
	}
	...
	//----------------------------------------------------------------------
	// 「フォーム関連」Sheet へ送信 (silent fail — メール送信フローに影響を与えない)
	//----------------------------------------------------------------------
	if (defined('SPENDA_GAS_URL') && SPENDA_GAS_URL) {
		sendToGoogleSpreadsheet($_POST, $SPENDA_FORM_ID);
	}
	//----------------------------------------------------------------------
}
...
})(window,document,'script','dataLayer','GTM-MX5CQMN');</script>
...
function copyright(){
	echo '<a style="display:block;text-align:center;margin:15px 0;font-size:11px;color:#aaa;text-decoration:none" href="https://spendacorp/" target="_blank">- SPENDA -</a>';
}

//----------------------------------------------------------------------
//  reCAPTCHA v3 検証関数
//----------------------------------------------------------------------
function verifyRecaptcha($token, $secret_key, $score_threshold = 0.5) {
	if (empty($token)) {
		return false;
	}
	...
	if ($response['success'] && $response['score'] >= $score_threshold) {
		return true;
	}
...
//----------------------------------------------------------------------
//  「フォーム関連」Sheet 連携関数 (spenda forms-to-sheets)
//----------------------------------------------------------------------
//  - SPENDA_GAS_URL / SPENDA_FORM_SECRET は spenda-config.php に定義
//  - 認証は共有シークレット方式 (GAS Script Properties の FORM_SHARED_SECRET と一致必須)
//  - silent fail: GAS が落ちてもメール送信フローには影響を与えない
//  - CURLOPT_FOLLOWLOCATION=true 必須 (GAS は script.googleusercontent.com にリダイレクト)
//----------------------------------------------------------------------
function sendToGoogleSpreadsheet($postData, $form_id) {
	$get = function($key) use ($postData) {
		if (!isset($postData[$key])) return '';
		$v = $postData[$key];
	...
	$payload = [
		'source'   => 'php-mail',
		'form_id'  => $form_id,
		'form_url' => isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : 'https://spendacorp.com/contact.html',
		'fields'   => [
			'name'    => $get('お名前'),
			'company' => $get('会社名'),
			'email'   => $get('Email'),
